...
public:
    ...
    void Create(Direct3D* direct_3d, const std::wstring& name, uint32_t NumElements, uint32_t ElementSize,
   ...
   /* void Create(const std::wstring& name, uint32_t NumElements, uint32_t ElementSize,
    ...
    void CreatePlaced(Direct3D* direct_3d, const std::wstring& name, ID3D12Heap* pBackingHeap, uint32_t HeapOffset, uint32_t NumElements, uint32_t ElementSize,
        const void* initialData = nullptr);
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE CreateConstantBufferView( Direct3D* direct_3d, uint32_t Offset, uint32_t Size ) const;
...
protected:
    ...
    D3D12_RESOURCE_DESC DescribeBuffer(Direct3D* direct_3d);
    virtual void CreateDerivedViews(Direct3D* direct_3d) = 0;
...
{
public:
    virtual void CreateDerivedViews(Direct3D* direct_3d) override;
};
...
public:
    ...
    ByteAddressBuffer& GetCounterBuffer(Direct3D* direct_3d) { return m_CounterBuffer; }

    const D3D12_CPU_DESCRIPTOR_HANDLE& GetCounterSRV(Direct3D* Context);
    const D3D12_CPU_DESCRIPTOR_HANDLE& GetCounterUAV(Direct3D* Context);
