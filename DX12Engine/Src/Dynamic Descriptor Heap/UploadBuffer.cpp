...
void UploadBuffer::Create(Direct3D* direct_3d, const std::wstring& name, size_t BufferSize )
{
    Destroy();
    ...
    ASSERT_SUCCEEDED(direct_3d->GetD3D12Device()->CreateCommittedResource(&HeapProps, D3D12_HEAP_FLAG_NONE, &ResourceDesc,
...
{
    CD3DX12_RANGE range = CD3DX12_RANGE(begin, std::min<size_t>(end, m_BufferSize));
