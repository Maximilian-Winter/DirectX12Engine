...
ID3D12DescriptorHeap* DescriptorAllocator::RequestNewHeap(ID3D12Device* d3d12Device, D3D12_DESCRIPTOR_HEAP_TYPE Type)
{
    ...
    d3d12Device->CreateDescriptorHeap(&Desc, MY_IID_PPV_ARGS(&pHeap));
    sm_DescriptorHeapPool.emplace_back(pHeap);
    return pHeap.Get();
}

D3D12_CPU_DESCRIPTOR_HANDLE DescriptorAllocator::Allocate(ID3D12Device* d3d12Device, uint32_t Count )
{
    if (m_CurrentHeap == nullptr || m_RemainingFreeHandles < Count)
    {
        m_CurrentHeap = RequestNewHeap(d3d12Device, m_Type);
        m_CurrentHandle = m_CurrentHeap->GetCPUDescriptorHandleForHeapStart();
        m_RemainingFreeHandles = sm_NumDescriptorsPerHeap;

        if (m_DescriptorSize == 0)
            m_DescriptorSize = d3d12Device->GetDescriptorHandleIncrementSize(m_Type);
    }
...
void DescriptorHeap::Create( ID3D12Device* d3d12Device, const std::wstring& Name, D3D12_DESCRIPTOR_HEAP_TYPE Type, uint32_t MaxCount )
{
    ...
    d3d12Device->CreateDescriptorHeap(&m_HeapDesc, IID_PPV_ARGS(m_Heap.ReleaseAndGetAddressOf()));
    ...
    m_DescriptorSize = d3d12Device->GetDescriptorHandleIncrementSize(m_HeapDesc.Type);
