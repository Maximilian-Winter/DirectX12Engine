...
public:
    ...
    LinearAllocationPage* RequestPage(Direct3D* context);
    LinearAllocationPage* CreateNewPage(Direct3D* context, size_t PageSize = 0 );
    ...
    // "large" pages.
    void FreeLargePages(Direct3D* context, uint64_t FenceID, const std::vector<LinearAllocationPage*>& Pages );
...
public:
    ...
    DynAlloc Allocate(Direct3D* direct_3d, size_t SizeInBytes, size_t Alignment = DEFAULT_ALIGN );

    void CleanupUsedPages( Direct3D* direct_3d, uint64_t FenceID );
...
private:
    ...
    DynAlloc AllocateLargePage(Direct3D* direct_3d, size_t SizeInBytes );
