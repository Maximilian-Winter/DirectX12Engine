...
//--------------------------------------------------------------------------------------
static HRESULT CreateTextureFromDDS( _In_ Direct3D* direct_3d,
                                     ...
                                     _In_ D3D12_CPU_DESCRIPTOR_HANDLE textureView )
{
...

    {
        ...
        if ( SUCCEEDED(hr) )
        {
            hr = CreateD3DResources( direct_3d->GetD3D12Device().Get(), resDim, twidth, theight, tdepth, mipCount - skipMip, arraySize,
                                     format, forceSRGB,
        ...
        if (SUCCEEDED(hr))
        {
            ...
            direct_3d->InitializeTexture(DestTexture, subresourceCount, initData.get());
        }
    }
    ...
    DDS_ALPHA_MODE* alphaMode )
{
    ...
    if (!direct_3d->GetD3D12Device().Get() || !ddsData)
    {
        return E_INVALIDARG;
    }
    ...
    HRESULT hr = CreateTextureFromDDS(direct_3d,
    ...
    DDS_ALPHA_MODE* alphaMode )
{
    ...
    if (!direct_3d->GetD3D12Device().Get() || !fileName)
    {
        return E_INVALIDARG;
    }
    ...
    hr = CreateTextureFromDDS(direct_3d,
