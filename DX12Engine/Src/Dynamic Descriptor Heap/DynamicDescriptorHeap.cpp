...
ID3D12DescriptorHeap* DynamicDescriptorHeap::RequestDescriptorHeap(Direct3D* owningContext, D3D12_DESCRIPTOR_HEAP_TYPE HeapType)
{
    std::lock_guard<std::mutex> LockGuard(sm_Mutex);

    uint32_t idx = HeapType == D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER ? 1 : 0;

    while (!sm_RetiredDescriptorHeaps[idx].empty() && owningContext->IsFenceComplete(sm_RetiredDescriptorHeaps[idx].front().first))
    {
    ...
    else
    {
        ...
        owningContext->GetD3D12Device()->CreateDescriptorHeap(&HeapDesc, MY_IID_PPV_ARGS(&HeapPtr));
...
DynamicDescriptorHeap::DynamicDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE HeapType): m_DescriptorType(HeapType)
{
}

DynamicDescriptorHeap::DynamicDescriptorHeap(Direct3D* OwningContext, D3D12_DESCRIPTOR_HEAP_TYPE HeapType)
    : m_OwningContext(OwningContext), m_DescriptorType(HeapType)
{
    m_CurrentHeapPtr = nullptr;
    m_CurrentOffset = 0;
    m_DescriptorSize = OwningContext->GetD3D12Device()->GetDescriptorHandleIncrementSize(HeapType);
}
...
void DynamicDescriptorHeap::InitDynamicDescriptorHeap(Direct3D* OwningContext)
{
    m_OwningContext = OwningContext;
    m_CurrentHeapPtr = nullptr;
    m_CurrentOffset = 0;
    m_DescriptorSize = OwningContext->GetD3D12Device()->GetDescriptorHandleIncrementSize(m_DescriptorType);
}
...
inline ID3D12DescriptorHeap* DynamicDescriptorHeap::GetHeapPointer()
{
    if (m_CurrentHeapPtr == nullptr)
    {
        ...
        m_CurrentHeapPtr = RequestDescriptorHeap(m_OwningContext, m_DescriptorType);
...
void DynamicDescriptorHeap::DescriptorHandleCache::CopyAndBindStaleTables( 
    Direct3D* context, D3D12_DESCRIPTOR_HEAP_TYPE Type, uint32_t DescriptorSize,
    ...
    void (STDMETHODCALLTYPE ID3D12GraphicsCommandList::*SetFunc)(UINT, D3D12_GPU_DESCRIPTOR_HANDLE))
{
    ...
    for (uint32_t i = 0; i < StaleParamCount; ++i)
    {
        ...
        while (_BitScanForward64(&SkipCount, SetHandles))
        {
            ...
            {
                context->GetD3D12Device()->CopyDescriptors(
    ...
    void (STDMETHODCALLTYPE ID3D12GraphicsCommandList::*SetFunc)(UINT, D3D12_GPU_DESCRIPTOR_HANDLE))
{
    ...
    HandleCache.CopyAndBindStaleTables(m_OwningContext, m_DescriptorType, m_DescriptorSize, Allocate(NeededSize), CmdList, SetFunc);
}
...
D3D12_GPU_DESCRIPTOR_HANDLE DynamicDescriptorHeap::UploadDirect( D3D12_CPU_DESCRIPTOR_HANDLE Handle )
{
    ...
    m_OwningContext->GetD3D12Device()->CopyDescriptorsSimple(1, DestHandle, Handle, m_DescriptorType);
