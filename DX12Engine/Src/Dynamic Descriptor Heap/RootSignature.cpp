...
void RootSignature::Finalize(Direct3D* direct_3d, const std::wstring& name, D3D12_ROOT_SIGNATURE_FLAGS Flags)
{
    if (m_Finalized)
        return;
    ...
    RootDesc.Desc_1_1.NumParameters = m_NumParameters;
    RootDesc.Desc_1_1.pParameters = reinterpret_cast<const D3D12_ROOT_PARAMETER1*>(m_ParamArray.get());
    RootDesc.Desc_1_1.NumStaticSamplers = m_NumSamplers;
    RootDesc.Desc_1_1.pStaticSamplers = static_cast<const D3D12_STATIC_SAMPLER_DESC*>(m_SamplerArray.get());
    RootDesc.Desc_1_1.Flags = Flags;
    ...
    if (firstCompile)
    {
        ...
        HRESULT hr = D3DX12SerializeVersionedRootSignature(&RootDesc, D3D_ROOT_SIGNATURE_VERSION_1_1, pOutBlob.GetAddressOf(), pErrorBlob.GetAddressOf());
        if (FAILED(hr))
        {
            if (pErrorBlob)
            {
        ...
        ASSERT_SUCCEEDED(direct_3d->GetD3D12Device()->CreateRootSignature(1, pOutBlob->GetBufferPointer(), pOutBlob->GetBufferSize(),
