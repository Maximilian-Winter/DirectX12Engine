...
public:
    ...
    void CreateFromMemory(Direct3D* direct_3d, ByteArray memory, TextureManager::eDefaultTexture fallback, bool sRGB);
...
namespace TextureManager
{
    ...
    ManagedTexture* FindOrLoadTexture(Direct3D* direct_3d, const wstring& fileName, eDefaultTexture fallback, bool forceSRGB )
    {
        ...
        tex->CreateFromMemory(direct_3d, ba, fallback, forceSRGB);
...
void ManagedTexture::CreateFromMemory(Direct3D* direct_3d, ByteArray ba, TextureManager::eDefaultTexture fallback, bool forceSRGB)
{
    if (ba->size() == 0)
    {
        m_hCpuDescriptorHandle = TextureManager::DefaultTextures::Get()->GetDefaultTexture(fallback);
    }
    else
    {
        // We probably have a texture to load, so let's allocate a new descriptor
        m_hCpuDescriptorHandle = direct_3d->AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

        if ( SUCCEEDED( CreateDDSTextureFromMemory(direct_3d, (const uint8_t*)ba->data(), ba->size(),
        ...
        {
            direct_3d->GetD3D12Device()->CopyDescriptorsSimple(1, m_hCpuDescriptorHandle, TextureManager::DefaultTextures::Get()->GetDefaultTexture(fallback),
...
D3D12_CPU_DESCRIPTOR_HANDLE TextureRef::GetSRV() const
{
    if (m_ref != nullptr)
        ...
        return TextureManager::DefaultTextures::Get()->GetDefaultTexture(TextureManager::kMagenta2D);
}


TextureRef TextureManager::LoadDDSFromFile(Direct3D* direct_3d, const wstring& filePath, eDefaultTexture fallback, bool forceSRGB )
{
    return FindOrLoadTexture(direct_3d, filePath, fallback, forceSRGB);
}

TextureRef TextureManager::LoadDDSFromFile(Direct3D* direct_3d, const string& filePath, eDefaultTexture fallback, bool forceSRGB )
{
    return LoadDDSFromFile(direct_3d, Utility::UTF8ToWideString(filePath), fallback, forceSRGB);
}
