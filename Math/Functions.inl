...
#undef CREATE_SIMD_FUNCTIONS
	...
	AffineTransform OrthoInvert( const AffineTransform& xform )
	{
		Matrix3 basis = Transpose(xform.GetBasis());
		return AffineTransform( basis, basis * -xform.GetTranslation() );
	}

	OrthogonalTransform Invert( const OrthogonalTransform& xform )	 { return ~xform; }

	Matrix4 Transpose( const Matrix4& mat ) { return Matrix4(XMMatrixTranspose(mat)); }
	Matrix4 Invert( const Matrix4& mat ) { return Matrix4(XMMatrixInverse(nullptr, mat)); }

	Matrix4 OrthoInvert( const Matrix4& xform )
	{
